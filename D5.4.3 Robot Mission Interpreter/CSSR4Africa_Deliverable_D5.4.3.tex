...
Figure~\ref{fig:interaction_diagram} illustrates the communication architecture of the Robot Mission Interpreter. The central node, shown in red, coordinates with eight peripheral ROS nodes shown in orange. The bidirectional communication is represented by arrows: solid arrows indicate service calls or action goals from the interpreter to the nodes, while dashed arrows topic messages sent from the nodes to the interpreter.
...
Each Leaf Node is implemented as a C++ class that inherits from either \texttt{\small BT::SyncActionNode}, \texttt{\small BT::StatefulActionNode} or \texttt{\small BT::ConditionNode}, depending on whether it represents an Action (Synchronous or Asynchronous) or a Condition node. The class definition includes the following components:
...
\end{lstlisting}
signature for synchronous tasks or 
\begin{lstlisting}[style=cppstyle]
    BT::StatefulActionNode(const std::string& name, const NodeConfig& config)
\end{lstlisting}
%\texttt{\small BT::SyncActionNode(const std::string\& name, const NodeConfig\& config)}
signature for asynchronous tasks, while that for a Condition node follows the 
...
\end{lstlisting}
signature. During initialization, the constructor sets up necessary ROS components, such as service and action clients and topic subscribers, to facilitate the node's functionality.

\subparagraph{2. Task Initiation}
%- - - - - - - - - - - - - - - - - - - - - 
The task initiation is handled by either of two methods

\begin{lstlisting}[style=cppstyle]
    BT::NodeStatus tick()
\end{lstlisting}
for synchronous tasks, or 
\begin{lstlisting}[style=cppstyle]
...
These two methods encapsulates the core logic of each Leaf Node and are invoked during behavior tree traversal. Serving as the primary execution method for both nodes, \texttt{\small tick()} performs the node's specific functionality, which may include modifying Blackboard values, sending service or goal requests to CSSR4Africa ROS nodes, or executing custom actions based on messages received from subscribed topics.

These methods returns one of three possible states:
\begin{itemize}
...
\begin{itemize}
    \item Class member variables shared between the constructor and the task initiation methods.
...
\begin{figure}[htbp]
    \dirtree{%
        ...
        .3 action.
        .4 gestureExecutionPerformGesture.action.
        .4 robotNavigationSetGoal.action.
        .4 robotNavigationSetPose.action.
        .4 speechEventRecogniseSpeech.action.
        .4 textToSpeechSayText.action.
        .3 msg.
...
\subsubsection{Action Definition Files}
%- - - - - - - - - - - - - - - - - - - - - 
Located in action/ directory, the action definition files are as follows:
\begin{itemize}
    \item \texttt{\small srv/\textnormal{gestureExecutionPerformGesture.action}}: Action definition for the
\\ \texttt{\small gestureExecution/perform\_gesture} server.
\item \texttt{\small srv/\textnormal{robotNavigationSetGoal.action}}: Action definition for the \texttt{\small robotNavigation/set\_goal} server.
\item \texttt{\small srv/\textnormal{textToSpeechSayText.action}}: Action definition for the \texttt{\small textToSpeech/say\_text} server.
\item \texttt{\small srv/\textnormal{speechEventRecogniseSpeech.action}}: Action definition for the 
\\ \texttt{\small speechEvent/recognise\_speech\_action} server.
\end{itemize}
...
\newpage
\subsection{Action goals sent}
The node interacts with the action servers detailed in the table below. Each entry specifies the action server name, the message format required, the expected effects or observations, and references to the corresponding deliverables that provide comprehensive explanations of the arguments passed and the return values from each request.
...
\begin{longtable}[c]{|p{6.4cm}|p{2.9cm}|p{3cm}|p{1.4cm}|}
    ...
    \texttt{\footnotesize gestureExecution/perform\_gesture} & \texttt{\footnotesize gesture\_type, gesture\_id, arm, deictic\_shape, gesture\_duration, bow\_nod\_angle, location\_x, location\_y, location\_z} & Invokes the gesture subsystem to perform a type of gesture with specified coordinates. The coordinates are only necessary if the \texttt{\footnotesize gesture\_type} is \texttt{\footnotesize deictic}. &D5.5.1 \\ \hline
    \texttt{\footnotesize robotNavigation/set\_goal} & \texttt{\footnotesize goal\_x, goal\_y, goal\_theta} & Invokes the navigation subsystem to move to the specified coordinates. &D5.5.4 \\ \hline
    \texttt{\footnotesize speechEvent/recognise\_speech\_action} & \texttt{\footnotesize language} & Sets the language for the ASR to transcribe utterances to &D5.5.4 \\ \hline
    \texttt{\footnotesize textToSpeech/say\_text} & \texttt{\footnotesize message, language} & Sends the text to be converted to an audio signal based on the language specified and played on the robot's loudspeakers. &D5.5.2.4 \\ \hline
...
A total of \textnormal{29 action and condition nodes} have been developed for the \texttt{\small behaviorController} module. These custom nodes are comprehensively listed in \textnormal{Table \ref{tab:implemented_mission_nodes}}. The table provides the following details for each:
...
There are six pairs of action nodes, each consisting of synchronous and asynchronous variants that perform the same underlying task. The asynchronous variants are non-blocking and interact with action servers by sending goals they can be composed to enable concurrent execution of tasks. In contrast, the synchronous nodes use service calls, are blocking in nature, and only one instance can execute at a time.
...
\begin{landscape}
{\footnotesize
\begin{longtable}[c]{|p{4.8cm}|p{1.4cm}|p{4cm}|p{3.8cm}|p{6.6cm}|}
    ...
    \texttt{\footnotesize DescribeExhibitSpeechAsync} & Action & Asynchronous variant of \texttt{\footnotesize DescribeExhibitSpeech} & None & \texttt{\footnotesize /textToSpeech/say\_text} \\ \hline
    ...
    \texttt{\footnotesize GetVisitorResponseAsync} & Action & Asynchronous variant of \texttt{\footnotesize GetVisitorResponse} & \texttt{\footnotesize /speechEvent/text} & None \\ \hline
    ...
    \texttt{\footnotesize NavigateAsync} & Action & Asynchronous variant of \texttt{\footnotesize Navigate} & None & \texttt{\footnotesize /robotNavigation/set\_goal} \\ \hline
    ...
    \texttt{\footnotesize PerformDeicticGestureAsync} & Action & Asynchronous variant of \texttt{\footnotesize PerformDeicticGesture} & None & \texttt{\footnotesize /gestureExecution/perform\_gesture} \\ \hline
    ...
    \texttt{\footnotesize PerformIconicGestureAsync} & Action & Asynchronous variant of \texttt{\footnotesize PerformIconicGesture} & None & \texttt{\footnotesize /gestureExecution/perform\_gesture} \\ \hline
    ...
    \texttt{\footnotesize SayTextAsync} & Action & Asynchronous variant of \texttt{\footnotesize SayText} & None & \texttt{\footnotesize /textToSpeech/say\_text} \\ \hline
...
\item [Version 1.9]~\\
References to the use of \texttt{\small ROS Actions} have been added where applicable \\
Modified the \texttt{\small /gestureExecution/perform\_gesture} service to include updates made to the service \\
Added \texttt{\small 4.10 Action Goals Sent} \\
Expanded Figure 2 and Figure 3 to update the file structures \\
Expanded \texttt{\small 4.11 Robot Mission Nodes} to include the action nodes that support ROS Actions \\
Tsegazeab Tefferi. \\                            
08 May 2026.
