...
This node provides and advertises a service server at \texttt{/gestureExecution/perform\_gesture}, which is used to initiate the execution of a specified gesture. It relies on a package-specific service definition, \texttt{performGesture.srv}. The service definition includes the following fields:

\begin{longtable}[c]{|l|p{3.5cm}|l|l|}
    \caption{Fields in the \texttt{performGesture.srv} of the gestureExecution node.} \label{tab:Gesture.msg_field} \\
    \hline
    \rowcolor{gray!30}
...
\footnotesize{\texttt{arm}} & \footnotesize{\texttt{LEFT, RIGHT, EITHER}} & \footnotesize{\texttt{String}} & \footnotesize{}  \\ \hline
\footnotesize{\texttt{deictic\_shape}} & \footnotesize{\texttt{PALM\_UPWARDS, PALM\_DOWNWARDS}} & \footnotesize{\texttt{String}} & \footnotesize{}  \\ \hline
...
\begin{longtable}[c]{|l|p{3.5cm}|l|l|}
    \caption{Fields in the \texttt{gesture.action} of the gestureExecution node.} \label{tab:gesture.action_field} \\
    \hline
    \rowcolor{gray!30}
    \footnotesize{\textbf{Field}} & \footnotesize{\textbf{Field Value}} & \footnotesize{\textbf{FIeld Type}} & \footnotesize{\textbf{Units}}\\ \hline
    \endhead % header for subsequent pages
...
If the goal is successful, the response is “1”; if it is unsuccessful, it is “0”. The action is initiated by the \texttt{ behaviorController} node.
...
\begin{longtable}[c]{|l|p{1.5cm}|p{7cm}|}
...
\small{\texttt{nod}} & \small{\texttt{true}} or \texttt{false} & \small{Specifies whether to run the nod gestures test} \\ \hline
 ...
 \small{\texttt{interface}} & \small{\texttt{service}} or \texttt{action} & \small{Specifies which ROS interfacing mechanism to use during tests.} \\ \hline
...
\item [Version 1.3]~\\
Updated the service definition to reflect the added improvement to the ROS node \\
Added ``Actions Supported'' and updated the ``File Organization'' under ``5. Implementation'' \\
Updated the unit tests configuration to reflect the added support for ``ROS Actions'' \\
Corrected wrong references to ``.msg'' files that were present
Tsegazeab Tefferi. \\                                   
15 May 2026.
